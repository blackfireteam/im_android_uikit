...
public class ChatRoomEditTodFragmentPresenter extends DynamicPresenter<ChatRoomEditTodFragment.ViewImpl> {
    ...
    @UiThread
    public ChatRoomEditTodFragmentPresenter(@NonNull ChatRoomEditTodFragment.ViewImpl view) {
        super(view);
        MSIMManager.getInstance().addSessionListener(mSessionListener);
        mChatRoomStateChangedBatchQueue.setConsumer(objects -> ChatRoomEditTodFragmentPresenter.this.onChatRoomStateChangedInternal());
    ...
    private boolean reInit() {
        final ChatRoomEditTodFragment.ViewImpl view = getView();
        if (view == null) {
